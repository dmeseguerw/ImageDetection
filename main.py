...
def check_matches(list1,pts,rad):
    ...
    for i in list1:
        if(check_point(i,pts,rad)): trueCtr = trueCtr + 1
...
def rect_center(ref_point, center):
    ...
    new_ref_point = [[center[0] - x,center[1] - y],[center[0] + x,center[1] + y]]
...
def analyze_all(fpath,sel, ref_point):
    ...
    for image in templates:
        ...
        if len(matches) > 25:
            ...
            ref_point_aux = rect_center(ref_point,[x2,y2])
            ...
            if val or val:
                ...
                if sel == '2':
                    ...
                    img_rect = cv2.rectangle(img_rect2, ref_point[0], ref_point[1], (0, 255, 0), 2)
                    img_rect = cv2.rectangle(img_rect1, ref_point_aux[0], ref_point_aux[1], (0, 255, 0), 2)
                    cv2.imshow("Keypoint matches", img_rect1)
                    cv2.imshow("Keypoint matches", img_rect2)
                    cv2.waitKey(0)

                # Show matches:
                    img5 = cv2.drawMatches(img1_circle, kp1, img2_circle, kp2, matches[:len(kp2)-1], None,
                                        flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
                    cv2.imshow("Keypoint matches", img5)
                    cv2.waitKey(0)
            else:
                os.remove(image)
                print("Removed by out of area " + image)
        else:
            os.remove(image)
            print("Removed cuz no matches" + image)
